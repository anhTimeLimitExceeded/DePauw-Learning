...
public class CodeRunnerServiceIntegrationTest {
    ...
    public void testCreateThread() {
        ...
        CodeRunner codeRunner = spy(new CodeRunnerImpl("test", messageHeaders, compilerService,
        ...
        verify(messagingTemplate,
                times(1))
                        .convertAndSendToUser(eq("test"), eq("/topic/runner/status"),
    ...
    public void testCreateMultipleThreads() {
        ...
        CodeRunner codeRunnerFirst = spy(new CodeRunnerImpl("test1", messageHeaders,
                compilerService, messagingTemplate, clock));

        CodeRunner codeRunnerSecond = spy(new CodeRunnerImpl("test2", messageHeaders,
                compilerService, messagingTemplate, clock));
    ...
    public void testAddsInput() {
        ...
        doAnswer(invocation -> {
            ...
            return null;
        }).when(messagingTemplate).convertAndSendToUser(eq("test"), eq("/topic/runner/status"),
                any(), same(messageHeaders));

        when(codeRunnerFactory.createCodeRunner(eq("test"), any(), any(), any()))
                .thenReturn(codeRunner);

        List<String> source = Collections.singletonList(TestData.getInputIdentitySource());

        codeRunnerService.createThread("test", source, messageHeaders, compilerService,
                messagingTemplate);

        codeRunnerService.addInput("test", "Test input\n");
